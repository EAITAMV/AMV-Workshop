...
/**
 * Moves the RedBot forward until it detects and obstacle, then executes an 
 * algorithm which tries to avoid the obstacle
 */
void moveRedBot() {
    driveForward(); 
    ...
    if (detectedLoadingPt()) {  // RedBot detected a loading point
    ...
    if (detectedObstacle()) {  // RedBot detected an obstacle
        motors.brake(); // Stop the motors
        playTone("detected obstacle"); // Beep to indicate detected obstacle
        reverse();
        pivotLeft(); // or pivotRight() - ensure speed is set so that the RedBot pivots 90 degrees in either direction
        driveForward();
        delay(1000);
        pivotRight(); // or pivotLeft()
    }
}
