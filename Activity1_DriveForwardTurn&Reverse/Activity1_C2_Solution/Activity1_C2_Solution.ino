...
/**
 * Implements RedBotMotors methods to turn both wheels so that the RedBot 
 * executes an about 180 degree turn to face the opposite direction
 */
void uTurn() {
  motors.rightMotor(100);    // Turn CW at motorPower of 100
  motors.leftMotor(100);     // Turn CW at motorPower of 100
  delay(1500);     // delay for longer so that the RedBot turns 180 degrees
  motors.brake(); // brake() motors
  delay(1000);    // delay for 1000 ms
}

/**
 * Uses the functions above to move the RedBot so moves forward and does a 
 * U-turn 
 */
void moveRedBot() {
  driveForward();
  ...
  // Method 1: 
  // Can use pivotRight()/pivotLeft() functions to do a U-Turn with delay
  // values increased so the RedoBot pivots longer to turn 180 degrees
  // pivotLeft();

  // Method 2: 
  // Create and implement a new U-turn function similar to pivotLeft/Right 
  // functions with a longer delay
  // uTurn();
}
