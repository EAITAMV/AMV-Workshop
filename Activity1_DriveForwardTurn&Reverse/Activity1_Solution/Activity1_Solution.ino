...
/**
 * Drives the RedBot forward and backwards as well as pivots the RedBot to 
 * the left and right
 */
void moveRedBot() {
  // Move the RedBot forward for one second then stop (shown as class example)
  driveForward();

  // Pivot the RedBot to the right (shown as class example)
  pivotRight();
  
  // Move the RedBot backward (student exercise)
  reverse();

  // Pivot the RedBot to the left (student exercise)
  pivotLeft();
}
...
void setup() {
  ...
  pinMode(BUTTON_PIN, INPUT_PULLUP); // configures the button as an INPUT
