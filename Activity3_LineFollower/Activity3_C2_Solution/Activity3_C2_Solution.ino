 ...
 *  
 * This activity programs the RedBot to follow a dark line on a light 
 * background or a light line on a dark background
 *
 * 04 Feb 2022 R. Danam  
 *****************************************************************************/
#include <RedBot.h>
...
/*****************************************************************************/
/**
 * Commands the RedBot to follow a dark line on a light background
 */
void moveRedBot() {
    ...
    if (veeringLeft()) { // Replace the if condition with your code!
        turnRight(driveSpeed);
    }
    // turn left if veering too far to the right of the line
    if (veeringRight()) { // Replace the if condition with your code!
        turnLeft(driveSpeed);
    }

    // RedBot detected a loading point
    if (detectedLoadingPt()) { 
    ...
    if (detectedObstacle()) { 
        motors.brake(); // Stop the motors
        playTone("detected obstacle"); // Beep the robot buzzer
        delay(5000); // delay for five seconds
    }
}
